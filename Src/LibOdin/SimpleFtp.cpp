...
BOOL CSimpleFtp::UploadFile(CString strLocalFilePath, CString strRemoteFilePath, BOOL bResume)
{
	...
	//전송기본정보(전송중인 파일명, 전송시작시간, 전송현재시간 전송할 데이터의 총 크기 현재 전송된 데이터 크기
	TransferInfo_In st_TransferInfo_In;
	//전송기본정보를 이용하여 전송속도, 남은시간 등의 추가정보를 구함
	TransferInfo_Out st_TransferInfo_Out;
	...
	while(!feof(fp))//로컬파일을 다 읽을때까지 반복
	{
		...
		{	
			...
			*Buff += dwSend;//MAX_BUFFER_SIZE
			...
			{
				GetTransferInfo(st_TransferInfo_In, st_TransferInfo_Out);
				//TRACE("File Size : %d,  Cur Size : %d,  Up Speed : %d,  percent : %d\n", st_TransferInfo_Out.nTotFileSize, st_TransferInfo_Out.nTransferSize, st_TransferInfo_Out.nSpeed / 1024, st_TransferInfo_Out.nPercent);
				::SendMessage(hCallWnd, WM_SHOW_UPLOAD_STATE, 0, (LPARAM)&st_TransferInfo_Out);
				PUMP_MESSAGES();
			}
